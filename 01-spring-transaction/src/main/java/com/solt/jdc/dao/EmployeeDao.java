...
public class EmployeeDao {
    ...
    public void saveEmployee(Employee employee){
        ...
        int numbersOfRecordsInserted = jdbcTemplate.update("insert into employee(employee_id, first_name, last_name, email, phone_number, hire_date, salary) "+
                "values (?, ?, ?, ?, ?, ?, ?)",
                employee.getId(),
        ...
        if (numbersOfRecordsInserted == 1){
            ...
            throw new IllegalArgumentException(String.format("Expected 1 record to be inserted, " +
                    "instead retrieved [%d] number of records inserted", numbersOfRecordsInserted));
        }
    }
