package com.solt.jdc.annotationcontrollermethod.controller;
...
public class Example03MatrixVariable {
    ...
    public String example03C(@MatrixVariable(pathVar = "employee", name = "department") String department,
        ...
        return String.format("Received request department = [%s], cityMatrixVariable = [%s] \n", department, location);
    }

    // curl "http://localhost:8080/example03D/employees/id=1;name=John;city=NYC"
    @GetMapping("/example03D/employees/{employee}")
    @ResponseBody
    public String example03D(@MatrixVariable Map<String, String> matrixVariable){
        return String.format("Received request matrixVariable = [%s] \n", matrixVariable);
    }
    ...
    @ResponseBody
    public String example03E(@MatrixVariable Map<String, String> matrixVariable){
        return String.format("Received request matrixVariable = [%s] \n", matrixVariable);
    }
    ...
    @ResponseBody
    public String example03F(@MatrixVariable(pathVar = "employee") Map<String, String> employeeMatrixVariable,
                             @MatrixVariable(pathVar = "city") Map<String, String> cityMatrixVariable){
        return String.format("Received request employeeMatrixVariable = [%s], cityMatrixVariable = [%s] \n", employeeMatrixVariable, cityMatrixVariable);
    }
}
